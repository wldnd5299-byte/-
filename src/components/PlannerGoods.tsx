                  ...
                  <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="p-3.5 bg-sky-50/80 border border-sky-200 rounded-xl md:col-span-2">
                        <span className="text-xs font-black text-sky-950 block mb-2">80세 이상 (초고령층)</span>
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-xs">
                          <div className="p-2 bg-white rounded border border-sky-200 text-center font-bold">
                            <span className="text-[10px] text-slate-400 block">1위</span>
                            <span className="text-rose-700">암 (16.3%)</span>
                          </div>
                          <div className="p-2 bg-white rounded border border-sky-200 text-center font-bold">
                            <span className="text-[10px] text-slate-400 block">2위</span>
                            <span className="text-sky-800">폐렴 (11.8%)</span>
                          </div>
                          <div className="p-2 bg-white rounded border border-sky-200 text-center font-bold">
                            <span className="text-[10px] text-slate-400 block">3위</span>
                            <span className="text-slate-800">심장질환 (10.5%)</span>
                          </div>
                          <div className="p-2 bg-white rounded border border-sky-200 text-center font-bold">
                            <span className="text-[10px] text-slate-400 block">4위</span>
                            <span className="text-slate-800">뇌혈관 (7.3%)</span>
                          </div>
                          <div className="p-2 bg-white rounded border border-sky-200 text-center font-bold">
                            <span className="text-[10px] text-slate-400 block">5위</span>
                            <span className="text-amber-800">알츠하이머 (5.5%)</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {activeReportSubTab === 'sales' && (
                <div className="space-y-4">
                  <div className="p-5 bg-emerald-50 border border-emerald-200 rounded-2xl space-y-3">
                    <h4 className="text-base font-black text-emerald-950 flex items-center gap-2">
                      <Award className="w-5 h-5 text-emerald-600" />
                      <span>설계사 대면 상담 및 보장 제안 활용 셀링 포인트</span>
                    </h4>

                    <div className="space-y-3 pt-1 text-xs text-emerald-900 leading-relaxed font-medium">
                      <div className="p-3.5 bg-white rounded-xl border border-emerald-200">
                        <strong className="text-emerald-950 font-black block text-sm mb-1">
                          1. 3대 질병(암·뇌·심) 통합 보장의 절대적 필요성
                        </strong>
                        통계청 발표 결과 사망 원인의 42.6%가 암, 심장질환, 폐렴 3가지에 집중되어 있습니다. 특히 40대 이상부터 암 사망률이 급증하므로, 3대 질병 진단비 및 수술비 비갱신형 통합 특약 설계를 적극 안내해야 합니다.
                      </div>
                      ...
                      <div className="p-3.5 bg-white rounded-xl border border-emerald-200">
                        ...
                        남성은 암 사망률(215.1명)이 여성(133.7명)의 1.6배에 달하고 간 질환(21.8명) 위험이 높습니다. 반면 여성은 알츠하이머 치매(32.8명) 및 고혈압성 질환 사망 위험이 현저히 높으므로, 성별 맞춤 담보를 구체적 통계 수치와 함께 스크립트화할 수 있습니다.
                      </div>

                      <div className="p-3.5 bg-white rounded-xl border border-emerald-200">
                        <strong className="text-emerald-950 font-black block text-sm mb-1">
                          3. 초고령화(80세 이상) 대비 치매간병 및 호흡기 특약
                        </strong>
                        80세 이상 사망 비중이 54.1%로 역대 최고치를 기록했습니다. 80세 이상에서는 폐렴(11.8%)과 알츠하이머(5.5%)가 사망원인 2위와 5위를 차지하므로, 노후 재가간병/치매진단비 및 질병입원일당 보장 소구가 대면 개척의 핵심입니다.
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-4 bg-slate-100 border-t border-slate-200 flex items-center justify-between">
              <span className="text-xs text-slate-500 font-bold hidden sm:inline">
                자료출처: {
                  ...
                  Number(selectedReportId) === 6 ? '국가암지식정보센터 · 보건복지부 (국가암통계공시)' :
                  Number(selectedReportId) === 5 ? '국가암지식정보센터 · 보건복지부 (국가암통계공시)' :
                  Number(selectedReportId) === 4 ? '국가암정보센터 · 보건복지부 (국가암통계공시)' :
                  Number(selectedReportId) === 3 ? '국립암센터 · 보건복지부 (공식자료)' :
                  Number(selectedReportId) === 2 ? '보건복지부 등록 장애인 통계 & 약관분석 (공식자료)' :
                  '통계청 사회통계국 인구동향과 (2025년 발표)'
                }
              </span>
