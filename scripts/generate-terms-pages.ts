...
`;
  ...
  // 4. Generate /terms/index.html with full internal links to all active insurers and coverage pages
  const activeInsurers = INSURER_TERMS_LIST.filter(
    ins => (INSURER_SUBTABS[ins.id] || []).length > 0
  );
  const activeInsurersCount = activeInsurers.length;
  const totalCoveragesCount = activeInsurers.reduce(
    (sum, ins) => sum + (INSURER_SUBTABS[ins.id] || []).length,
    0
  );
  const activeInsurerNames = activeInsurers.map(ins => ins.name).join(', ');

  const allInsurersCardsHtml = activeInsurers
    .map(ins => {
      ...
      <main style="max-width: 1100px; margin: 24px auto; padding: 0 16px;">
        ...
        <section style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; margin-bottom: 24px;">
          ...
          <p style="margin: 0 0 16px 0; font-size: 14px; color: #64748b; line-height: 1.6;">
            ${activeInsurerNames} 등 주요 ${activeInsurersCount}개 손해보험사의 암·뇌·심·수술비 등 총 ${totalCoveragesCount}개 담보별 약관 질병코드 분류표를 대조하고 검색할 수 있습니다.
          </p>
          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 16px; font-size: 13px; color: #475569; line-height: 1.6;">
            특정 질병코드가 기재된 담보 분류표를 찾으려면 <a href="/disease-code/" style="font-weight: 700; color: #123941; text-decoration: underline;">질병코드 검색</a>을 이용할 수 있습니다.
          </div>
        </section>

        <!-- ${activeInsurersCount} Insurers and ${totalCoveragesCount} Coverage Direct Links -->
        <section id="terms-insurers-directory">
          <h2 style="font-size: 18px; font-weight: 800; color: #0f172a; margin-bottom: 16px;">
            보험사별 담보 바로가기 (${activeInsurersCount}개 보험사 · ${totalCoveragesCount}개 담보)
          </h2>
